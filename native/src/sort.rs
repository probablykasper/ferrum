...
use crate::data::{Data, get_open_playlist_tracks};
...
pub fn sort<'a>(data: &'a mut Data, sort_key: &str) -> Result<(), &'static str> {
  ...
  if sort_key == "index" {
    ...
    match playlist {
      ...
      TrackList::Folder(_) | TrackList::Special(_) => {
        return Ok(())
      },
    }
  }
  ...
  data.open_playlist_track_ids.sort_by(|id_a, id_b| {
    ...
    match field {
      Some(TrackField::String) => {
        ...
        if str_a == "" && str_b == "" { return Ordering::Equal }
        if str_a == "" { return Ordering::Greater }
        if str_b == "" { return Ordering::Less }
