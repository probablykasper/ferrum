...
impl Library {
	...
	pub fn generate_id(&self) -> String {
		...
		for _ in 0..1000 {
			...
			if !self.trackLists.contains_key(&id) {
				return id;
			}
		}
		panic!("Error generating ID: Generated IDs already exist")
	}
	pub fn generate_next_track_id(&mut self) -> TrackID {
		let next_id = match self.last_track_id {
			Some(last_id) => last_id + 1,
			None => {
				let last_id = self.tracks.iter().map(|(id, _)| id).max();
				match last_id {
					Some(last_id) => last_id + 1,
					None => 0,
				}
		...
		self.last_track_id = Some(next_id);
		next_id
	}
	...
	pub fn get_tracklist(&self, id: &TrackListID) -> Result<&TrackList> {
		self.trackLists.get(id).context("Playlist ID not found")
	}
	pub fn get_tracklist_mut(&mut self, id: &TrackListID) -> Result<&mut TrackList> {
	...
	pub fn get_parent_id(&self, id: &TrackListID) -> Option<String> {
...
pub static TRACK_ID_MAP: RwLock<Vec<TrackID>> = RwLock::new(Vec::new());
