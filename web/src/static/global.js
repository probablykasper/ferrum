...
(function commonFunctions() {
    ...
    window.insertAfter = function(arr, i, val) {
...
(function colResize() {
    ...
    var mouseDown, mousePosX, mousePosStartX, oldWidth, oldWidthNext, nextCol;
    var flexibleCols, availWidth, nextColClass, currentColClass, currentCol;
    document.addEventListener("mousedown", function(e) {
...
(function colRearrange() {
    ...
    var mouseDown, mousePosX, mousePosStartX, currentCol, nextCol, prevCol, moveCount;
...
(function dropFiles() {
    ...
    addEvents(window, ["dragenter"], function(e) {
        if (containsFiles(e) && loggedIn) show();
    });
    // hide
...
(function setupPlayer() {
    ...
    rangeSlider.create(progressSlider, {
        ...
        value: 0,
        onInit: function() {
            progressSliderElement = document.querySelector(".player .rangeSlider.progress-slider")
        },
        onSlideStart: function() {
            progressMouseDown = true;
            progressSliderElement.classList.add("mouse-down");
        },
        onSlideEnd: function(value) {
            progressMouseDown = false;
            audio.currentTime = value * audio.duration;
            progressSliderElement.classList.remove("mouse-down");
        }
    });
        ...
        if (icon) {
            ...
            if (cl.contains("play-pause")) {
                if (player.classList.contains("paused")) {
                    ...
                    queue.push(pageTracks[0]);
                    setSrc(queue[0].trackId);
                    play();
                    for (var i = 1; i < pageTracks.length; i++) {
                        autoQueue.push(pageTracks[i]);
                    }
                }
    ...
    rangeSlider.create(volumeSlider, {
        ...
        onSlide: function(value) {
            audio.volume = value;
            updateVolIcon(value);
            volumeContainer.classList.add("mouse-down");
        },
        onSlideEnd: function(value) {
            ...
            updateVolIcon(value);
            volumeContainer.classList.remove("mouse-down");
        }
    });

    var volIcon = document.querySelector(".player svg.icon.volume");
    updateVolIcon(audio.volume);
    // toggle volume on/off by clicking icon
    function getSavedVolume() {
        return localStorage.getItem("savedVolume");
    }
    ...
    volIcon.addEventListener("click", function() {
        if (audio.volume != 0) {
            ...
            audio.volume = 0;
            updateVolIcon(0);
            volumeSlider.rangeSlider.update({
                value: 0
            });
        } else if (getSavedVolume()) {
    ...
    function updateVolIcon(value) {
        if (value == 0) {
            if (volIcon.dataset.loudness != "0") {
                ...
                volIcon.innerHTML = '<path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z"></path><path d="M0 0h24v24H0z" fill="none"></path>';
            }
        } else if (value <= 0.5) {
            if (volIcon.dataset.loudness != "2") {
                volIcon.dataset.loudness = "2";
                volIcon.innerHTML = '<path d="M18.5 12c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM5 9v6h4l5 5V4L9 9H5z"></path><path d="M0 0h24v24H0z" fill="none"></path>';
            }
        } else if (value > 0.5) {
            if (volIcon.dataset.loudness != "3") {
                volIcon.dataset.loudness = "3";
                volIcon.innerHTML = '<path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"></path><path d="M0 0h24v24H0z" fill="none"></path>';
            }
        }
    }
})();
