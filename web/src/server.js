...
function old() {
    ...
    if      (!done && S(path).startsWith("/css/")) read(path.substr(1), "text/css");
        ...
        if (path == "/") path = "/index";
        if      (fs.existsSync(`pages${path}/index.html`)) path = `pages${path}/index.html`;
        else if (fs.existsSync(`pages${path}.html`))       path = `pages${path}/index.html`;
        else path = `pages/404.html`;
